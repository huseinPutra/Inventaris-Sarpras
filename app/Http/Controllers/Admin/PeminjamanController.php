...
class PeminjamanController extends Controller
{
    public function index()
    {
    	...
    	return view('Admin.Peminjaman.index',compact('Pegawai','Inventaris','Peminjaman'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($input['tanggal_max_kembali'] <= " ") {
            return back()->with(['message' => 'Tanggal Max Kembali Wajib Diisi ','type' => 'warning']);
        }
    ...
    public function edit($id){
        ...
        return view('Admin.Peminjaman.edit',compact('data','barang','peminjam','id'));
    }
    ...
    public function update(Request $req,$id){
        ...
        if ($input['id_inventaris_before']) {
            ...
            $rollback = Inventaris::where('id',$input['id_inventaris_before'])->update([
                ...
                'jumlah' => $readrollback['barang_baik'] + $readrollback['jumlah_rusak'] + $input['jumlah_before']
            ]); 
        }
        ...
        $update = Inventaris::where('id',$input['id_inventaris'])->update([
            'barang_baik' => $read['barang_baik'] - $input['jumlah'],
            'jumlah' => ($read['barang_baik'] + $read['jumlah_rusak']) - $input['jumlah'],
        ]);
    ...
    public function view($id){
        ...
        return view('Admin.Peminjaman.view',compact('data'));
    }   
    ...
    public function approvep(Request $req, $id){
        ...
        $barang = Inventaris::where('id',$datadetail['id_inventaris'])->update([
            ...
            'jumlah' => $read['barang_baik'] + $read['jumlah_rusak']  - $datadetail['jumlah'],
        ]);

        if ($barang) {
            return redirect()->route('Admin.Peminjaman.index')->with(['message' => 'Data Peminjaman berhasil diapprove','type' => 'success']);
        }else{
    ...
    public function maxkembali($id){
        
        return view('Admin.Peminjaman.tanggal_max_kembali',compact('id'));
    }


    public function laporanpeminjaman()
    {
        $peminjaman = Peminjaman::join('detail__peminjaman','peminjaman.id','=','detail__peminjaman.id_peminjaman')->get();
